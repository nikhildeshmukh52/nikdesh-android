...
class MainViewModel: ViewModel() {
    ...
    fun saveToExternalStorage(context: Context, bitmap: Bitmap): Boolean {
        val externalDirectory = context.getExternalFilesDir(null)
        val directory = File(externalDirectory, "AndroidExternalStorage")
        if(!directory.exists()) {
            directory.mkdir()
        }
    ...
    suspend fun readImageFromExternalStorage(): Boolean {
